...
public:
	...
	UniParams polar_hidden_linear;
	FourParams seg_att_eval_polar_concat;
...
public:
	bool initial(HyperParams& opts, AlignedMemoryPool* mem = NULL){
		...
		polar_hidden_linear.initial(opts.polarityHiddenSize, opts.polarityDim, true, mem);
		seg_att_eval_polar_concat.initial(opts.concatHiddenSize, opts.wordHiddenSize, opts.attDim * 3, opts.evalCharHiddenSize * 3 * 3, opts.polarityHiddenSize, true, mem);
	...
	void exportModelParams(ModelUpdate& ada){
		...
		polar_hidden_linear.exportAdaParams(ada);
		seg_att_eval_polar_concat.exportAdaParams(ada);
